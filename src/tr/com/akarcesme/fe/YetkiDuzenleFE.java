...
public class YetkiDuzenleFE extends JDialog implements FeInterfaces {
	...
	public JPanel initPanel() {
		...
		/*
		 * JList yetkiList = new JList(); yetkiList.setListData(new
		 * YetkilerDal().GetAll().toArray()); JScrollPane pane = new
		 * JScrollPane(yetkiList);
		 * pane.setBorder(BorderFactory.createTitledBorder("Düzenlenecek Liste"));
		 * yetkiList.setSelectedIndex(0); yetkiAdiField.addKeyListener(new KeyListener()
		 * {
		 * 
		 ...
		 * }
		 * 
		 * @Override public void keyReleased(KeyEvent e) { yetkiList.setListData(new
		 * YetkilerDal().GetSearchYetki(yetkiAdiField.getText()).toArray());
